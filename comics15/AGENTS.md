...
| 前端入口 | `frontend/index.html` + `frontend/src/main.ts` | Vue3 + Pinia + Vue Router |
| 页面级视图 | `frontend/src/pages/` | `WaterfallPage`, `DirectoryPage`, `ReaderPage`, `ToolsPage` |
| Vue 组件 | `frontend/src/components/` | 阅读器组件 + `components/tools/` 工具页组件 |
| 路由/URL 构建 | `frontend/src/router/index.ts` | `/series/{series}/dir|read/{path}`、`/tools`；中文路径分段编码 |
| 媒体 URL 构建 | `frontend/src/services/media-url.ts` + `api.ts` | `/hq_image`、`/lq_image`、`/video`；LQ 缺失 204 回退 HQ |
| 工具页状态/API | `frontend/src/stores/tools-store.ts` + `frontend/src/services/tools-api.ts` | 工具列表、执行记录、轮询 |
...
| `WaterfallPage` | Vue page | `frontend/src/pages/WaterfallPage.vue` | 首页瀑布流，漫画系列浏览 |
| `DirectoryPage` | Vue page | `frontend/src/pages/DirectoryPage.vue` | 章节树、封面懒加载、目录渲染 |
| `ReaderPage` | Vue page | `frontend/src/pages/ReaderPage.vue` | 漫画阅读视图，图片/视频浏览 |
...
| `router` | Vue Router module | `frontend/src/router/index.ts` | Vue Router、URL 构建、中文路径分段编码 |
...
npm run lint
npm run lint:fix
vitest run src/utils/route-path.test.ts
...
| `docs/backend-api.md` | 后端 API 契约与响应格式 | 修改后端 API 或前端对接前 |
| `docs/frontend-design.md` | 前端页面流、组件树、状态设计 | 改页面流/组件结构前 |
...
- 测试文件：后端在 `backend/src/test/...`；前端 `*.test.ts` 与源文件同目录；Go 工具暂无 `_test.go`。
