...
def serve_low_quality_image(series_name, chapter_path_id, filename):
    ...
    print(f"INFO: Serving content (Quality 15): {series_name}/{chapter_path_id}/{filename}")
...
# ===========================
# 4.1️⃣ 提供高质量图片 (直接提供原始文件)
# (保持不变)
# ===========================
@app.route('/hq_image/<series_name>/<path:chapter_path_id>/<filename>')
def serve_high_quality_content(series_name, chapter_path_id, filename):
    
    print(f"INFO: Serving High Quality (Original) content: {series_name}/{chapter_path_id}/{filename}")
    ...
    lower_filename = filename.lower()
    # 检查是否为图片文件（避免视频文件请求到此路由）
    if not lower_filename.endswith(SUPPORTED_IMAGE_EXTENSIONS):
        print(f"WARNING: Non-image file requested on hq_image route: {filename}")
        return "Only image files supported on hq_image route.", 400
    
    # 直接发送原始文件，性能最佳，确保画质最高
    try:
        ...
        print(f"FATAL: Failed to serve original file {file_path}: {e}")
