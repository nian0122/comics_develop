...
def serve_content(series_name, chapter_path_id, filename):
    ...
    if filename.lower().endswith(('.jpg', '.jpeg', '.png', '.webp')):
        ...
        try:
            ...
            # 关键：图片保存为 JPEG，质量为 20
            img.save(output, format='JPEG', quality=20)
            output.seek(0)
            
            print(f"SUCCESS: Image processed and served as JPEG (quality=10).")
...
if __name__ == '__main__':
    # 在 Docker Compose 环境中，HOST 应该设置为 '0.0.0.0'
    host = '0.0.0.0'
    port = 500
    ...
    # 提示本地访问地址
    if host == '0.0.0.0':
         print(f"INFO: Access via: http://{local_ip}:{port} (Local IP)")
         print(f"INFO: Access via: http://127.0.0.1:{port} (Loopback)")


    # 在开发模式下，设置 use_reloader=True 可以利用卷挂载实现热重载
